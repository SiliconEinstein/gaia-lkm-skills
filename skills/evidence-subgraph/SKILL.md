...
Every successful invocation of this skill must leave a conforming run-folder on disk. The contract — directory layout, structured `evidence_graph.json` schema, the four pair JSON files (`contradictions.json` / `equivalences.json` / `cross_validation.json` / `dismissed_pairs.json`), the verbatim `raw/` payloads, the source-pointer convention (RFC 6901 JSON Pointer), and the pre-success self-check — is specified canonically in `references/run-folder-output-contract.md`. The judgement criteria that drive whether a contradiction or cross-validation candidate is promoted into `contradictions.json` / `cross_validation.json` or dismissed into `dismissed_pairs.json` are specified in `references/pair-classification.md`. Read both reference files before producing any output, and run the contract's §8 self-check before declaring the task complete. Failing any check in §8 is a hard failure.
