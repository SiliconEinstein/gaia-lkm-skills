...
extracts every `claim(...)` and `deduction(...)` call plus the `PRIORS` dict,
and emits the two audit-trail files required by the `$gaia-package` contract:
...
Notes:
    - Schema follows `$gaia-package/references/audit-log.md` v1 paper-extract subset:
      `package_initialized`, `accepted_claim` (with claim_kind), `accepted_deduction`,
      `prior_added`. No candidate-handling events.
...
def parse_paper_file(path: Path) -> tuple[list[tuple[str, dict]], list[tuple[str, list[str], str | None, dict]]]:
    """Return (claims, deductions).
...
def build_events(
    ...
    for var_name, kw in claims:
        ...
        events.append(make_event(
            "accepted_claim",
            {
                ...
                "weak_types": kw.get("weak_types", []) or [],
    ...
    # 3. accepted_deduction per deduction()
    ...
    # 4. prior_added per priors.py entry
...
def main(argv: list[str] | None = None) -> int:
    ...
    claims, deductions = parse_paper_file(paper_file)
    events = build_events(pkg_dir, paper_key, claims, deductions, priors)
    audit_md = build_audit_md(pkg_dir, paper_key, claims, deductions, priors)

    if args.dry_run:
        print(f"DRY RUN — would write to {pkg_dir / 'artifacts/paper-extract'}")
        print(f"  graph_growth_log.jsonl: {len(events)} events")
        print(f"  mapping_audit.md: {len(claims)} claims + {len(deductions)} deductions")
        return 0
    ...
    with open(audit_path, "w") as fh:
        fh.write(audit_md)
    print(f"✓ Wrote {len(claims)} claims + {len(deductions)} deductions → {audit_path}")
