---
name: formalize
description: Use when turning a single academic paper into a Gaia knowledge package via a four-phase analytical workflow — Phase 1 extracts conclusions / motivation / open questions / cross-conclusion logic graph, Phase 2 reconstructs each conclusion's reasoning chain, Phase 3 audits 9 argument-pattern weak-point types (`measurement` / `causal` / `model` / `statistical` / `generalization` / `comparative` / `formal` / `computational` / `external`) and highlights, Phase 4 emits the Gaia DSL package via `gaia author`. Gated by an upfront suitability check that skips review / survey / perspective papers. Phase 1b best-effort cross-grounds against LKM's existing knowledge graph via `$lkm-search`'s `/search` endpoint. Sibling to `$lkm-explorer` (LKM-driven workflow). Trigger phrases include 'formalize this paper into Gaia', 'paper.md → gaia', 'review 这文章', 'turn this paper into a knowledge package'.
---
