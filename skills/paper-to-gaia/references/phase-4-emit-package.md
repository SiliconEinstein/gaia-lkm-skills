...
`package-skeleton.md`, populated according to `mapping-contract.md`. After
emission, the package must be ready for `gaia compile` and
`gaia check --hole .`.
...
Label rules (recap from `mapping-contract.md`):
...
- Initialize `artifacts/paper-extract/graph_growth_log.jsonl` and append a
  `package_initialized` event (see `package-skeleton.md` →
  "graph_growth_log.jsonl schema"). Establish your `actor_id` (e.g.
  `paper-to-gaia-<short-uuid>`) and `seq` counter starting at 1.
...
5. **Weak-point leaf claims section** — one `claim(...)` per Phase 3 weak
   ...
   graph instead — see `mapping-contract.md §2a`.
...
7. **Open questions section (optional, opt-in)** — only emit
   ...
   Pipeline B does not extract open questions; this is a paper-to-gaia
   extension.
...
conclusion's posterior — see `mapping-contract.md §3`), follow the
additive scheme:
...
Use the template from `package-skeleton.md`. Populate every section:
...
- Conclusions table — one row per conclusion, with the working-note id,
  ...
  bound to this conclusion — see `mapping-contract.md §2a`),
...
- Weak points table — one row per weak-point label, with label, the single
  ...
  affects but which are **not** wired into BP — see `mapping-contract.md
...
one JSON object following the schema in `package-skeleton.md` →
"graph_growth_log.jsonl schema". Required event sequence:
...
Use the template from `package-skeleton.md`. Fields:

- `name`: the kebab-case package name.
- `[tool.gaia].uuid`: a freshly minted UUID4.
- `[tool.gaia].generated_by`: `"paper-to-gaia"`.
