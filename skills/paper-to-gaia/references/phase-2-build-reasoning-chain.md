...
`@unknown_<n>` (bare `@key`, **no brackets** — `gaia compile` rejects bracketed `[@unknown_n]` as an unresolvable strict reference; bare `@key` is opportunistic) and note the gap so Phase 4 can record it in
`mapping_audit.md`.
