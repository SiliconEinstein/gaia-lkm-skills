...
A literature review benefits from reproducing or adapting figures and data tables from the source papers — that is convention in the field. This skill follows the convention on a **best-effort** basis, bounded by what the chain payload actually carries.
...
```
| section | citation in review | what was referenced | source paper (DOI) | why not reproduced |
```
