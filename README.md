...
- **`skills/lkm-explorer/`** — contract-driven LKM exploration → Gaia knowledge package per the upstream Gaia spec. Maps raw LKM match/evidence/source payloads into Gaia DSL via a five-step contradiction-driven workflow. Two modes: `batch` (fresh package) and `refresh` (extend or repair an existing package in place).
- **`skills/formalize/`** — paper-driven sibling to `$lkm-explorer`. Reads a single physics paper Markdown and emits a Gaia knowledge package per the upstream Gaia spec via a four-phase analytical workflow (extract conclusions → reconstruct reasoning chain → audit weak points → emit DSL). Phase 1b cross-grounds the paper against LKM's existing graph via `$lkm-api`'s `/search` reverse trace (best-effort; skips silently when the paper isn't in the corpus).
