...
3. **Raw LKM Search Task** — `$lkm-search` directly when the user only wants to search, inspect, or compare LKM claims/reasoning without Gaia formalization.
...
1. Clone the repo.
2. Read `skills/orchestrator/SKILL.md` first; follow its `$lkm-search`, `$lkm-explorer`, `$formalize`, `$evidence-subgraph`, `$scholarly-synthesis` references on demand. For Gaia DSL syntax, CLI command reference, and package layout, follow upstream `SiliconEinstein/Gaia` docs `docs/for-users/`.
