...
- `docs/for-users/cli-commands.md` — `gaia init/compile/check/infer/run render/...` 完整命令参考
...
按照 upstream `SiliconEinstein/Gaia` 仓库 `docs/for-users/quick-start.md` 的安装指引安装 Gaia 本体，并通过 `gaia --version` / `gaia compile --help` 验证。
...
5. 运行 upstream 质量门控：gaia compile && gaia check --hole && gaia infer
6. 报告结果
```
...
4. **质量门控** — `gaia compile && gaia check --hole && gaia infer`（见 upstream `docs/for-users/cli-commands.md`）
...
| Step 5 | `references/step-5-emit-and-handoff.md` | Emit 包 + handoff（runs upstream `gaia compile`/`check`/`infer`） |
...
│   └── dismissed/
└── .gaia/                          # gaia compile / infer 输出
    ├── ir.json
    ├── beliefs.json
    └── inquiry/
```

包结构、`mapping_audit.md` 通用 column / `graph_growth_log.jsonl` v1 schema 均由 upstream 拥有（见 `SiliconEinstein/Gaia` `docs/for-users/`）；`artifacts/lkm-discovery/` 子目录的具体文件由 `skills/lkm-explorer/references/package-skeleton.md` 规定。
...
`$formalize` 的 audit dir 是 `artifacts/paper-extract/`（与 `$lkm-explorer` 的 `artifacts/lkm-discovery/` 区分；两者 schema 兼容，按 upstream `graph_growth_log.jsonl` v1 spec emit）。
