...
├── src/<import>/
│   ├── __init__.py
│   └── paper_<key>.py              # 每篇论文一个 sibling（含 claims / derives / cross-paper 算子 / register_prior）
...
包结构由 upstream 拥有（见 `SiliconEinstein/Gaia` `docs/for-users/`）；module-routing 约定（一篇论文一个 `paper_<key>.py` sibling）由 `skills/lkm-explorer/references/package-skeleton.md` 规定。
...
| Phase 4 | `references/phase-4-emit-package.md` | Emit 包源代码（`paper_<key>.py` 通过 `gaia author` cli-as-client） |
