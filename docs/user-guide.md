...
| Step 4 | `references/step-4-supports-priors-and-review.md` | Support 搜索（每 frontier claim ≥2 distinct queries）、shared-factor 提取、leaf priors via `gaia author register-prior` |
...
- **LKM `support` 因子** → Gaia `derive(...)`，warrant strength intent encoded in `--rationale` prose（v0.5 canonical 形态，替代 legacy `support(...)` strategy；engine `derive(...)` 不接受 `metadata=` kwarg，per upstream Mendel/Galileo walkthrough pattern）
...
- **Empty-content premise**（chain 内部）：可以用 placeholder string + `todo="revisit when LKM corpus populates this premise"` 保留 factor-derived `derive(...)`
...
LKM-explorer 输出的标准 Gaia 包结构（按 upstream Gaia 知识包 spec）：

```
<domain>-gaia/
├── pyproject.toml
├── references.json                 # 可选；JSON object keyed by citation key, CSL-JSON entry shape
├── src/<import>/
│   ├── __init__.py                 # claims / derives / contradicts / equals / exclusives / observes / notes / questions 都写在这里
│   └── priors.py                   # 唯一的 sibling module；register_prior 调用专用，via `gaia pkg add-module --name priors --imports register_prior`
...
包结构与 module-routing 都由 upstream 拥有：见 `SiliconEinstein/Gaia` `docs/for-users/` 以及 `gaia example mendel` / `gaia example galileo` shipping walkthroughs（canonical pattern：除 `priors.py` 外所有 emissions 都写 `__init__.py`）。
...
| Phase 3 | `references/phase-3-review-weak-points.md` | 审计 weak points 和 highlights；标定 `prior_probability` / `p1` / `p2`。也包含 **Phase 1b** LKM 反向溯源（best-effort） |
| Phase 4 | `references/phase-4-emit-package.md` | Emit 包源代码 via `gaia author` cli-as-client（emissions 写 `__init__.py`；`register_prior` 写 `priors.py` sibling） |
